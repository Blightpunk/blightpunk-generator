...
def gerar_pdf_com_fundo(texto, imagem_fundo_path, imagem_fardo_path):
    ...
    c.drawImage(fardo, 14.2*cm, 25.7*cm, width=3.8*cm, height=5.5*cm, preserveAspectRatio=True, mask='auto')
...
if st.session_state.gerado:
    ...
    # Botão flutuante funcional via st.button + rerun
    st.markdown(
        """
        <style>
        .botao-flutuante {
    ...
    with st.container():
        if st.button("🔄 Revelar Outro Fardo", key="reload_btn"):
            st.session_state.gerado = False
