...
def gerar_pdf_com_fundo(texto, imagem_fundo_path, imagem_fardo_path):
    ...
    c.drawImage(fardo, 14*cm, 19.5*cm, width=6.2*cm, height=8.8*cm, preserveAspectRatio=True, mask='auto')
    ...
    for linha in texto.split('\n'):
        partes = []
        while c.stringWidth(linha, "EBGaramond", 13) > max_width:
            corte = len(linha)
            while c.stringWidth(linha[:corte], "EBGaramond", 13) > max_width and corte > 0:
                corte -= 1
...
if st.button("📜 Revelar Fardo"):
    ...
    st.session_state.personagem = {
        "idade_d4": idade_d4, "idade": idade, "fardo_id": fardo_id,
        "fardo_nome": fardo_nome, "arcano": arcano, "img_path": img_path,
        "atributos": atributos, "habilidades": habilidades,
        "cortina_d4": cortina_d4, "cortina": cortina,
        "estigmas": estigmas,
        "fortune_roll": fortune_roll, "fortune_etat": fortune_etat, "faixa": faixa,
        "plaie_roll": plaie_roll, "plaie": plaie,
        "obj_roll": obj_roll, "obj_nome": obj_nome,
        "obj_tipo": obj_tipo, "obj_dano": obj_dano, "obj_regra": obj_regra
    }
